...
- 인터랙션: 드래그 이동, 다중 선택(러버밴드 + Ctrl/Shift 클릭), 정렬/자동 레이아웃.
...
- **FR-TYP-1 (H) 컨테이너의 사용자 타입**
  - 설명: 배열/셋/맵의 원소·값 타입으로 struct를 허용한다(현재는 스칼라 struct만 지원).
...
- **FR-TYP-5 (L) 핀 색상 커스터마이즈**
  - 설명: 사용자 타입의 핀 색을 지정할 수 있다(현재는 이름 해시 자동 색).
  - AC: 타입 편집기에서 색을 지정하면 핀/링크에 반영된다.

- **FR-TYP-6 (M) 타입 선택기의 사용자 타입 노출**
  - 설명: 핀/프로퍼티/변수/함수 파라미터의 타입을 고르는 모든 선택 UI 는 빌트인 스칼라뿐
    아니라 현재 등록된 사용자 정의 타입(enum/struct/object)도 목록에 포함한다.
    추가로, `Object` 카테고리의 노드 클래스는 핀/필드 타입 선택기에 이름 기반 오브젝트
    타입(nominal object)으로 함께 노출한다(별도 Type 정의 없이 오브젝트를 노드 간 전달).
    (v1 사용자 정의 타입 요구 및 본 문서 2.1 "핀/프로퍼티 사용"의 명시적 정리.)
  - AC: 사용자 타입 정의 후 타입 선택기에 해당 타입이 나타나고 선택 시 핀/프로퍼티/변수/
    파라미터 타입으로 지정된다. Object 카테고리 노드 클래스도 핀 타입 목록에 나타난다.
...
- **FR-WASM-4 (H) 자체 완결 툴체인 번들**
  - 설명: wasm 노드 함수 빌드에 필요한 clang(wasm32)/wasm-ld 툴체인을 GAU 배포본에
    포함한다. 사용자는 별도 LLVM 설치 없이 앱 내에서 새 함수를 추가·빌드·적용할 수 있어야
    한다. 앱은 실행 파일 옆의 `tools/llvm/bin` 을 우선 탐색하고, 없으면 설정값(clangPath),
    시스템 LLVM, PATH 순으로 폴백한다.
  - AC: 툴체인이 번들된 배포본에서 LLVM 미설치 환경의 사용자가 함수 소스를 작성하고 Build
    하면 `.wasm` 이 생성되어 그래프에서 실행된다(외부 설치·설정 불필요).
