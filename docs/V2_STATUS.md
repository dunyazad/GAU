...
- 완료:
  ...
  - 파라미터/변수 타입 선택(Type 버튼): 빌트인 스칼라 + 등록된 사용자 타입(struct/enum/
    object) 순환(FR-TYP-6). 단 사용자 타입은 프로젝트 로드로만 등록됨 -- gau2 내 타입 저작
    UI(struct/enum 생성)는 아직 없음.
...
3. **타입 저작 UI (FR-TYP-6 후속)** — gau2 내에서 struct/enum 정의(v1 ClassEditorDialog
   상당). 현재 타입 선택기는 로드된 사용자 타입만 노출.
4. (사소) 코멘트 텍스트 인라인 편집.
